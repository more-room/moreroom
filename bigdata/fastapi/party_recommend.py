...
def get_theme_and_member_id_by_party_request(party_request_id, party_request_df, matched_users):
    try:
        ...
        # 이미 매칭된 유저의 요청일 경우 파티를 생성하지 않음
        if member_id in matched_users:
            print(f"멤버 {member_id}는 이미 매칭되었으므로 파티를 생성하지 않습니다.")
            return None, None  # 이미 매칭된 유저이므로 파티 생성 중단
...
def get_candidate_users(theme_id, party_request_df, matched_users):
    try:
        ...
        # 이미 매칭된 유저는 제외
        theme_users = [user for user in theme_users if user not in matched_users]  # 매칭된 유저 제외
...
def create_party(similarity_matrix, similar_users, target_member_id, matched_users, initial_threshold=0.8):
    try:
        ...
        while len(party_members) < 3 and threshold >= 0.3:  # 유사도가 0.3 미만이면 종료
            ...
            # 이미 매칭된 유저와 중복된 유저를 제외
            similar_members = [idx for idx in similar_members if similar_users_array[idx] not in added_members and similar_users_array[idx] not in matched_users]
        ...
        # 매칭이 완료된 후에만 매칭된 유저 추가 (파티 생성 후에만 matched_users에 추가)
        matched_users.update(party_members)
